...
`meal-library.html` is a single self-contained page that embeds all six meals behind
a home grid — tap a meal to load its full interactive card in place, "← All meals" to
go back. It's generated by `build-hub.js`, which copies the engine (CSS + render
functions) from a card and embeds each meal's `#meal-data` island, so the hub never
drifts from the standalone cards. Regenerate with `node build-hub.js` (run from this
directory). `apply-colorpass.js` is the transform that applied the color/macro pass
across all cards.
